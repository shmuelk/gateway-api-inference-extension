...
func (s *SLOAwareRouter) scoreWithoutPredictions(
	...
	for _, endpoint := range endpoints {
		prefixScore := s.getPrefixCacheScoreForPod(ctx, state, endpoint)
...
func (s *SLOAwareRouter) Score(ctx context.Context, state *schedulingtypes.CycleState, request *schedulingtypes.LLMRequest, endpoints []schedulingtypes.Endpoint) map[schedulingtypes.Endpoint]float64 {
	...
	for _, endpoint := range endpoints {
		prefixCacheScore := s.getPrefixCacheScoreForPod(ctx, state, endpoint)
		sloCtx.prefixCacheScoresForEndpoints[endpoint.GetMetadata().String()] = prefixCacheScore
	}
	...
	r := rand.New(source)
	predictions, err := s.generatePredictions(ctx, state, request, sloCtx, endpoints)
	if err != nil {
		...
		s.setSLOContextForRequest(request, sloCtx)
		return s.scoreWithoutPredictions(ctx, state, endpoints, r)
	}
	...
	// Check if all endpoints are invalid and all have running requests
	allEndpointsInvalid := (sloCtx.ttftSLO > 0 && sloCtx.avgTPOTSLO > 0)
	...
	selectedEndpoint := s.selectEndpointBasedOnStrategy(ctx, r, allPreds, posHeadroomEndpoints, negHeadroomEndpoints)
