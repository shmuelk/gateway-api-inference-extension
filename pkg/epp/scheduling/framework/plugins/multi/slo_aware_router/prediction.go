...
func (s *SLOAwareRouter) generatePredictions(ctx context.Context, state *schedulingtypes.CycleState, request *schedulingtypes.LLMRequest, sloCtx *sloRequestContext, candidateEndpoints []schedulingtypes.Endpoint) ([]endpointPredictionResult, error) {
...
func (s *SLOAwareRouter) updateRequestContextWithPredictions(sloCtx *sloRequestContext, predictions []endpointPredictionResult) {
	for _, pred := range predictions {
		if pred.Error == nil {
			endpointKey := pred.Endpoint.GetMetadata().String()
			...
			sloCtx.predictedTTFTForScheduling[endpointKey] = pred.TTFT
			sloCtx.predictedTPOTForScheduling[endpointKey] = pred.TPOT
		}
	}
}
